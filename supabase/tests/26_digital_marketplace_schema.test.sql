...
select ok((select bool_and(p.prosecdef) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='private' and (p.proname like '%digital_product%' or p.proname like '%marketplace%') and p.proname not like 'set_%' and p.proname not like 'freeze_%' and p.proname not like 'validate_%' and p.proname<>'assert_marketplace_payload_keys'),'private marketplace operations are security definer');
