...
select is((select count(*)::integer from information_schema.role_routine_grants where specific_schema='private' and grantee in ('anon','authenticated') and routine_name in ('affiliate_is_admin','log_affiliate_event','affiliate_subject_is_available','apply_affiliate_commission_adjustment')),0,'internal-only affiliate helpers have no client execution grants');
