...
select is((select count(*)::integer from pg_trigger where not tgisinternal and tgfoid='public.set_updated_at()'::regprocedure), 20, 'updated_at trigger is attached to twenty mutable tables');
