...
select is((select count(*)::integer from pg_trigger where not tgisinternal and tgfoid='public.set_updated_at()'::regprocedure), 26, 'updated_at trigger is attached to twenty-six mutable tables');
