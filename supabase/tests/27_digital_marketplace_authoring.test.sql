...
select results_eq(
 $$select (public.create_digital_product_license((select id from public.digital_products where slug='pack-authoring'),jsonb_build_object('kind','commercial','title','Licença comercial','terms_text','Licença comercial para uma pessoa, sem revenda ou redistribuição dos arquivos originais.','is_default',true))).version$$,
 $$values(1)$$,
 'first license receives version one'
);
select results_eq(
 $$select (public.publish_digital_product_license((select id from public.digital_product_licenses where product_id=(select id from public.digital_products where slug='pack-authoring') and kind='commercial'))).status::text$$,
 $$values('published'::text)$$,
 'license publishes through lifecycle RPC'
);
...
select results_eq(
 $$select (public.publish_digital_product((select id from public.digital_products where slug='pack-authoring'),4)).status::text$$,
 $$values('published'::text)$$,
 'complete product publishes'
);
...
select results_eq(
 $$select (public.unpublish_digital_product((select id from public.digital_products where slug='pack-authoring'),5)).status::text$$,
 $$values('draft'::text)$$,
 'published product can return to draft'
);
select lives_ok(
...
select results_eq(
 $$select kind::text from public.digital_product_licenses where status='published' and is_default$$,
 $$values('personal'::text)$$,
 'default license can switch without mutating terms'
);
select results_eq(
 $$select (public.archive_digital_product((select id from public.digital_products where slug='pack-authoring'),(select version from public.digital_products where slug='pack-authoring'))).status::text$$,
