...
begin
  ...
  if v_stream_found and p_event_sequence <= v_stream.last_event_sequence then
    ...
    insert into public.lesson_progress_events (
      id, user_id, aula_id, client_instance_id, auth_session_id,
      event_sequence, event_type, position_seconds, duration_seconds,
      calculated_progress_percent, resulting_completed, accepted,
      ignored_reason, resulting_revision, observed_at
    ) values (
      p_event_id, v_user_id, p_lesson_id, p_client_instance_id, v_session_id,
      p_event_sequence, p_event_type, p_position_seconds, p_duration_seconds,
      v_calculated_percent, v_progress.completada, false,
      'STALE_SEQUENCE', v_progress.revision, p_observed_at
    );
    return v_progress;
  end if;
  ...
  if v_progress_found then
    ...
    if v_result_completed then v_result_percent := 100; end if;
    ...
    insert into public.progresso_aulas (
      user_id, aula_id, completada, progresso_percentual, tempo_assistido,
      ultima_visualizacao, revision, last_event_id,
      last_event_received_at, last_client_instance_id
    ) values (
      v_user_id, p_lesson_id, v_result_completed, v_result_percent,
      v_result_position, v_now, 1, p_event_id, v_now, p_client_instance_id
    ) returning * into v_progress;
  end if;

  insert into public.lesson_progress_streams (
    user_id, aula_id, client_instance_id, auth_session_id,
    last_event_sequence, last_event_id, last_position_seconds,
    created_at, updated_at
  ) values (
    v_user_id, p_lesson_id, p_client_instance_id, v_session_id,
    p_event_sequence, p_event_id, p_position_seconds, v_now, v_now
  )
  ...
  insert into public.lesson_progress_events (
    ...
    p_event_id, v_user_id, p_lesson_id, p_client_instance_id, v_session_id,
    p_event_sequence, p_event_type, p_position_seconds, p_duration_seconds,
    v_calculated_percent, v_progress.completada, true,
    null, v_progress.revision, p_observed_at
  );
