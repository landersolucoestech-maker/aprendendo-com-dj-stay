-- FASE B12: qualify playback token columns that collide with RETURNS TABLE output names.
...
begin
  ...
  with revoked as (
    update public.playback_tokens pt
    set revoked_at = statement_timestamp()
    where pt.user_id = v_user_id
      and pt.auth_session_id = v_session_id
      and pt.lesson_media_id = v_media.id
      and pt.revoked_at is null
      and pt.expires_at > statement_timestamp()
    returning pt.id, pt.user_id, pt.lesson_media_id
  )
  insert into public.playback_events(
    playback_token_id, user_id, lesson_media_id, event_type, reason, details
  )
  select r.id, r.user_id, r.lesson_media_id, 'revoked', 'TOKEN_RENEWED', '{}'::jsonb
  from revoked r;
