...
as $$
  select
    private.has_any_role(array['admin'::public.app_role, 'owner'::public.app_role])
    or exists (
      select 1
...
revoke all on function private.can_access_lesson(uuid) from public, anon;
...
begin
  ...
  select completion_threshold_percent into target_threshold
  ...
  insert into public.lesson_progress (
    ...
    (select auth.uid()),
    target_lesson_id,
    least(greatest(target_progress_percent, 0), 100),
    greatest(target_watched_seconds, 0),
    target_progress_percent >= target_threshold,
    case when target_progress_percent >= target_threshold then now() else null end,
    greatest(target_last_position_seconds, 0),
    now()
  )
  ...
  set progress_percent = calculated.progress_percent,
      ...
      updated_at = now()
  from (
    select target_enrollment.id as enrollment_id,
      coalesce(round(100.0 * count(*) filter (where lp.completed) / nullif(count(*), 0), 2), 0) as progress_percent
    from public.lessons l
    join public.course_modules m on m.id = l.module_id
    left join public.lesson_progress lp
      on lp.lesson_id = l.id
     and lp.enrollment_id = target_enrollment.id
    where m.course_id = target_enrollment.course_id
      and l.is_required
      and l.is_published
  ) calculated
...
begin
  ...
  foreach module_id in array ordered_module_ids loop
    update public.course_modules
       set sort_order = position, updated_at = now()
     where id = module_id and course_id = target_course_id;
    position := position + 1;
  end loop;
end;
$$;
...
begin
  ...
  if target_course_id is null or not private.can_manage_course(target_course_id) then
    raise exception 'Insufficient permissions';
  end if;
  ...
  foreach lesson_id in array ordered_lesson_ids loop
    update public.lessons
       set sort_order = position, updated_at = now()
     where id = lesson_id and module_id = target_module_id;
    position := position + 1;
  end loop;
end;
$$;
...
create policy courses_public_or_authorized_select on public.courses for select to anon, authenticated
using (
  deleted_at is null and (
    status = 'published'
    or private.can_manage_course(id)
...
with check (
  ...
  and private.has_any_role(array['instructor'::public.app_role, 'admin'::public.app_role, 'owner'::public.app_role])
);
...
create policy modules_catalog_select on public.course_modules for select to anon, authenticated
using (
...
using (
  exists (select 1 from public.course_modules m where m.id = module_id and private.can_manage_course(m.course_id))
)
with check (
  exists (select 1 from public.course_modules m where m.id = module_id and private.can_manage_course(m.course_id))
);
...
using (
  visibility = 'preview' and exists (select 1 from public.lessons l where l.id = lesson_id and l.is_preview)
...
using (
  exists (select 1 from public.lessons l join public.course_modules m on m.id = l.module_id
          where l.id = lesson_id and private.can_manage_course(m.course_id))
)
with check (
  exists (select 1 from public.lessons l join public.course_modules m on m.id = l.module_id
...
using (user_id = (select auth.uid()) or exists (
  select 1 from public.enrollments e where e.id = enrollment_id and private.can_manage_course(e.course_id)
));

create policy certificates_select on public.certificates for select to authenticated
using (user_id = (select auth.uid()) or private.can_manage_course(course_id) or private.has_any_role(array['support'::public.app_role, 'admin'::public.app_role, 'owner'::public.app_role]));

create policy reviews_public_select on public.course_reviews for select to anon, authenticated
using (is_published or user_id = (select auth.uid()) or private.can_manage_course(course_id));
create policy reviews_student_insert on public.course_reviews for insert to authenticated
with check (user_id = (select auth.uid()) and exists (
  select 1 from public.enrollments e where e.id = enrollment_id and e.user_id = (select auth.uid()) and e.course_id = course_id
));
create policy reviews_student_update on public.course_reviews for update to authenticated
...
drop policy if exists course_images_instructor_manage on storage.objects;
create policy course_images_instructor_manage on storage.objects for all to authenticated
using (
...
drop policy if exists course_videos_authorized_read on storage.objects;
create policy course_videos_authorized_read on storage.objects for select to authenticated
using (
  bucket_id = 'course-videos'
  and exists (select 1 from public.lessons l where l.video_storage_path = name and private.can_access_lesson(l.id))
);

drop policy if exists course_videos_instructor_manage on storage.objects;
create policy course_videos_instructor_manage on storage.objects for all to authenticated
using (
  bucket_id = 'course-videos'
  and (storage.foldername(name))[1] ~* '^[0-9a-f-]{36}$'
  and private.can_manage_course(((storage.foldername(name))[1])::uuid)
...
drop policy if exists course_assets_authorized_read on storage.objects;
create policy course_assets_authorized_read on storage.objects for select to authenticated
using (
  bucket_id = 'course-assets'
  and exists (select 1 from public.lesson_assets a where a.storage_path = name and private.can_access_lesson(a.lesson_id))
);

drop policy if exists course_assets_instructor_manage on storage.objects;
create policy course_assets_instructor_manage on storage.objects for all to authenticated
using (
  bucket_id = 'course-assets'
  and (storage.foldername(name))[1] ~* '^[0-9a-f-]{36}$'
  and private.can_manage_course(((storage.foldername(name))[1])::uuid)
...
-- Migrate the current single-course prototype into the new domain.
do $$
declare
...
begin
  ...
  if owner_id is not null and to_regclass('public.modulos') is not null and to_regclass('public.aulas') is not null then
    insert into public.courses (
      id, title, slug, short_description, description, category, level,
      status, published_at, created_by
    ) values (
      default_course_id,
      'Aprendendo com DJ Stay',
      'aprendendo-com-dj-stay',
      'Curso de produção musical com DJ Stay.',
      'Conteúdo migrado da versão original da plataforma.',
      'Produção Musical',
      'all',
      'published',
      now(),
    ...
    values (default_course_id, owner_id, true)
    ...
    insert into public.lessons (
      id, module_id, title, slug, description, content_type, video_url,
      duration_seconds, sort_order, is_published, created_at, updated_at
    )
    select
      a.id,
      a.modulo_id,
      a.titulo,
      regexp_replace(lower(unaccent(coalesce(a.titulo, a.id::text))), '[^a-z0-9]+', '-', 'g'),
      a.descricao,
      'video'::public.lesson_content_type,
      a.video,
      coalesce(a.duracao, 0) * 60,
      a.ordem,
      true,
      a.created_at,
      a.updated_at
    ...
    if to_regclass('public.user_subscriptions') is not null then
      ...
      where us.status in ('active', 'paid', 'approved')
    ...
    if to_regclass('public.progresso_aulas') is not null then
      ...
      select
        ...
        p.user_id,
        p.aula_id,
        coalesce(p.progresso_percentual, 0),
        coalesce(p.tempo_assistido, 0),
        coalesce(p.completada, false),
        case when coalesce(p.completada, false) then coalesce(p.ultima_visualizacao, p.updated_at) end,
        coalesce(p.tempo_assistido, 0),
        coalesce(p.ultima_visualizacao, p.updated_at)
