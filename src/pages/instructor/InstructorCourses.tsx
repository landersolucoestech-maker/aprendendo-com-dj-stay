  ...
  return (
    <div className="p-5 md:p-8 space-y-6">
          ...
          <CardContent>
            <form onSubmit={submit} className="grid md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2 flex justify-end gap-2">
                ...
                <Button type="submit" disabled={createCourse.isPending}>{createCourse.isPending ? "Criando..." : "Criar curso"}</Button>
              </div>
            </form>
          </CardContent>
        </Card>
      ...
      {!isLoading && !courses?.length && (
        <Card className="glass-card border-white/10"><CardContent className="p-10 text-center"><BookOpen className="w-10 h-10 mx-auto mb-3 text-gray-500" /><p>Nenhum curso encontrado.</p></CardContent></Card>
      )}
          ...
          <Card key={course.id} className="glass-card border-white/10">
            <CardContent className="p-5 space-y-4">
              <div className="flex items-start justify-between gap-3">
                <div><h2 className="font-semibold text-lg">{course.title}</h2><p className="text-sm text-gray-400 mt-1 line-clamp-2">{course.short_description || "Sem descrição"}</p></div>
                <span className="text-xs rounded-full bg-white/10 px-2 py-1">{course.status}</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-sm text-gray-400">
                <span>{course.course_modules?.[0]?.count ?? 0} módulos</span>
                <span>{course.category || "Sem categoria"}</span>
                <span className="col-span-2">Atualizado em {formatDate(course.updated_at)}</span>
              </div>
