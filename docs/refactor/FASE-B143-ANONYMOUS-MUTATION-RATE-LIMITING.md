...
2. resolve a primeira origem válida de `x-forwarded-for`, com fallback para `cf-connecting-ip` e `x-real-ip`;
...
A primeira migration cria o segredo, o estado de quota, os helpers privados e os dois triggers. A segunda adiciona políticas `DENY ALL` explícitas para os papéis não privilegiados.
...
O arquivo `supabase/tests/anonymous_mutation_rate_limiting.test.sql` contém 20 asserções pgTAP para catálogo, privilégios, RLS, comportamento real sob `anon`, independência por origem, falha fechada, bypass de serviço, HMAC e expiração.

O verificador `scripts/check-anonymous-mutation-rate-limiting.mjs` está encadeado ao `typecheck` pelo contrato obrigatório de contatos. Ele bloqueia a remoção silenciosa das migrations, triggers, quotas, testes, privacidade do identificador e mensagens públicas.
